	...
	//BATCHING:
		...
		for (unsigned i = 0; i < meshes.size();)
		{
			...
			else if (meshes[i]->renderState)
			{
				...
				if (!batchFound)
				{
					meshBatches.push_back(new MeshBatch(meshes[i]->worldVertexArray.size(), meshes[i]->shaderIndex, meshes[i]->textureDataID, meshes[i]->drawMode, meshes[i]->blending, meshes[i]->lineWidth));
