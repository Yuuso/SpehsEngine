...
namespace se
{
	namespace rendering
	{
		...
		ModelData::ModelData() {}
		void ModelData::loadFromData(std::vector<Vertex3D> &_vertexArray, std::vector<GLushort> &_elementArray)
		{
			int index;
		...
		void ModelManager::loadModel(const std::string& _filepath, Mesh* _mesh)
		{
		...
		void ModelManager::loadOBJ(const size_t& _hash, Mesh* _mesh)
		{
		...
		void ModelManager::loadOBJFromFile(std::ifstream& _data, const size_t& _hash)
		{
			...
			while (std::getline(_data, line))
			{
				...
				if (line.substr(0, 2) == "v ")
				{
					...
					data->vertices.push_back(Vertex3D(vertex));
				}
				//Normals
