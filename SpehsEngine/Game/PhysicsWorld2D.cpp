			...
			for (unsigned cycle2 = cycle1 + 1; cycle2 < bodies.size(); cycle2++)
			{
				...
				if (circleCollision(bodies[cycle1]->position, bodies[cycle1]->circleRadius, bodies[cycle2]->position, bodies[cycle2]->circleRadius))
				{
					...
					if (bodies[cycle1]->numVertices > 10) //if circle
					{
						...
						{
							collisionResult = spehs::circleMTVCollision(collisionResults, bodies[cycle1]->position, bodies[cycle1]->circleRadius, bodies[cycle2]->position, bodies[cycle2]->circleRadius);
						}
						else
						{
							collisionResult = spehs::SATMTVCollision(collisionResults, bodies[cycle2]->vertexData, bodies[cycle2]->numVertices, bodies[cycle1]->position, bodies[cycle1]->circleRadius);
						}
					}
					else
					{
						...
						{
							collisionResult = spehs::SATMTVCollision(collisionResults, bodies[cycle1]->vertexData, bodies[cycle1]->numVertices, bodies[cycle2]->position, bodies[cycle2]->circleRadius);
						}
						else
						{
							collisionResult = spehs::SATMTVCollision(collisionResults, bodies[cycle1]->vertexData, bodies[cycle1]->numVertices, bodies[cycle2]->vertexData, bodies[cycle2]->numVertices);
						}
					}
					...
					if (collisionResult)
					{
						...
						{
							spehs::vec2 body1VelocityBefore = bodies[cycle1]->getVelocityAtPosition(collisionResults.points[i].position);
							spehs::vec2 body2VelocityBefore = bodies[cycle2]->getVelocityAtPosition(collisionResults.points[i].position);
							...
							if (relativeNormalVelocity < -ZERO_EPSILON) //Point are colliding
							{
								...
								spehs::vec2 correction = std::max(collisionResults.MTV.getLength() - slop, 0.0f) / (bodies[cycle1]->getInvMass() + bodies[cycle2]->getInvMass()) * percentage * collisionResults.points[i].normal;
								...
								spehs::vec2 rVecAP = spehs::vec2(-(collisionResults.points[i].position - bodies[cycle1]->centerOfMass).y, (collisionResults.points[i].position - bodies[cycle1]->centerOfMass).x);
								spehs::vec2 rVecBP = spehs::vec2(-(collisionResults.points[i].position - bodies[cycle2]->centerOfMass).y, (collisionResults.points[i].position - bodies[cycle2]->centerOfMass).x);
								spehs::vec2 tangent = spehs::vec2(-collisionResults.points[i].normal.y, collisionResults.points[i].normal.x);
