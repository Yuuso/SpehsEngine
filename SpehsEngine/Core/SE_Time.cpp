...
namespace se
{
	static_assert(detail::time_conversion_rate::second == std::chrono::high_resolution_clock::time_point::period::den, "TODO");
	static_assert(detail::time_conversion_rate::second == std::chrono::system_clock::time_point::period::den * 100, "TODO");
	...
	void Time::initialize()
	{
		...
		initialized = true;
		log::info("SpehsEngine time accuracy is " + std::to_string(detail::time_conversion_rate::second) + " ticks per second.");
	}
	...
	Time getProfilerTime()
	{
		...
		{
			return Time(TimeValueType(double(counter.QuadPart) / (queryPerformanceFrequency / double(detail::time_conversion_rate::second))));
		}
		else
		{
...
#else
#error getProfilerTime() implementation is missing.
#endif
		return getEpochTime();
	}
