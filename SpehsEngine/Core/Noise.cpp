...
namespace se
{
    ...
    {
        /**
        * Based on K.jpg's OpenSimplex 2, faster variant
        * CSharp implementation, from 2022.05.12 (e09a947)
        ...
        */
        ...
        static constexpr int64_t SEED_OFFSET_4D = 0xE83DC3E0DA7164DL;
        ...
        static constexpr double FALLBACK_ROTATE_3D = 2.0 / 3.0;
        static constexpr double ROTATE_3D_ORTHOGONALIZER = UNSKEW_2D;

        static constexpr float SKEW_4D = -0.138196601125011f;
        static constexpr float UNSKEW_4D = 0.309016994374947f;
        ...
        static constexpr int N_GRADS_2D = 1 << N_GRADS_2D_EXPONENT;
        static constexpr int N_GRADS_3D = 1 << N_GRADS_3D_EXPONENT;
        static constexpr int N_GRADS_4D = 1 << N_GRADS_4D_EXPONENT;

        static constexpr double NORMALIZER_2D = 0.01001634121365712;
        static constexpr double NORMALIZER_3D = 0.07969837668935331;
        ...
        static void init()
        {
            ...
            #define array_sizeof(_var) (sizeof(_var) / sizeof(_var[0]))
            ...
            for (int i = 0; i < array_sizeof(grad2); i++)
            {
                grad2[i] = (float)(grad2[i] / NORMALIZER_2D);
            }
            for (int i = 0, j = 0; i < array_sizeof(GRADIENTS_2D); i++, j++)
            {
                if (j == array_sizeof(grad2)) j = 0;
            ...
            for (int i = 0; i < array_sizeof(grad3); i++)
            {
                grad3[i] = (float)(grad3[i] / NORMALIZER_3D);
            }
            for (int i = 0, j = 0; i < array_sizeof(GRADIENTS_3D); i++, j++)
            {
                if (j == array_sizeof(grad3)) j = 0;
            ...
            for (int i = 0; i < array_sizeof(grad4); i++)
            {
                grad4[i] = (float)(grad4[i] / NORMALIZER_4D);
            }
            for (int i = 0, j = 0; i < array_sizeof(GRADIENTS_4D); i++, j++)
            {
                if (j == array_sizeof(grad4)) j = 0;
        ...
        */
        ...
        inline static float Grad(int64_t seed, int64_t xsvp, int64_t ysvp, float dx, float dy)
        {
        ...
        inline static float Grad(int64_t seed, int64_t xrvp, int64_t yrvp, int64_t zrvp, float dx, float dy, float dz)
        {
        ...
        inline static float Grad(int64_t seed, int64_t xsvp, int64_t ysvp, int64_t zsvp, int64_t wsvp, float dx, float dy, float dz, float dw)
        {
        ...
        inline static int FastRound(double x)
        {
        ...
        */
        static float Noise4_UnskewedBase(int64_t seed, double xs, double ys, double zs, double ws)
        {
            ...
            // Prime pre-multiplication for hash.
            int64_t xsvp = xsb * PRIME_X, ysvp = ysb * PRIME_Y, zsvp = zsb * PRIME_Z, wsvp = wsb * PRIME_W;
    ...
    NoiseGenerator::NoiseGenerator(int64_t _seed)
        : seed(_seed)
    {
        OpenSimplexNoise::init();
    }
    void NoiseGenerator::setSeed(int64_t _seed)
    {
        seed = _seed;
    }
    float NoiseGenerator::generate(double _x, double _y, NoiseType _type) const
    {
        return generate(_x, _y, 0.0, 0.0, _type);
    }
    float NoiseGenerator::generate(double _x, double _y, double _z, NoiseType _type) const
    {
        return generate(_x, _y, _z, 0.0, _type);
    }
    float NoiseGenerator::generate(double _x, double _y, double _z, double _w, NoiseType _type) const
    {
        switch (_type)
        {
            case NoiseType::OpenSimplex_2D_Default:                     return OpenSimplexNoise::Noise2(seed, _x, _y);
            case NoiseType::OpenSimplex_2D_ImproveX:                    return OpenSimplexNoise::Noise2_ImproveX(seed, _x, _y);
            case NoiseType::OpenSimplex_2D_UnskewedBase:                return OpenSimplexNoise::Noise2_UnskewedBase(seed, _x, _y);
            case NoiseType::OpenSimplex_3D_ImproveXY:                   return OpenSimplexNoise::Noise3_ImproveXY(seed, _x, _y, _z);
            case NoiseType::OpenSimplex_3D_ImproveXZ:                   return OpenSimplexNoise::Noise3_ImproveXZ(seed, _x, _y, _z);
            case NoiseType::OpenSimplex_3D_Fallback:                    return OpenSimplexNoise::Noise3_Fallback(seed, _x, _y, _z);
            case NoiseType::OpenSimplex_3D_UnrotatedBase:               return OpenSimplexNoise::Noise3_UnrotatedBase(seed, _x, _y, _z);
            case NoiseType::OpenSimplex_4D_ImproveXYZ_ImproveXY:        return OpenSimplexNoise::Noise4_ImproveXYZ_ImproveXY(seed, _x, _y, _z, _w);
            case NoiseType::OpenSimplex_4D_ImproveXYZ_ImproveXZ:        return OpenSimplexNoise::Noise4_ImproveXYZ_ImproveXZ(seed, _x, _y, _z, _w);
            case NoiseType::OpenSimplex_4D_ImproveXYZ:                  return OpenSimplexNoise::Noise4_ImproveXYZ(seed, _x, _y, _z, _w);
            case NoiseType::OpenSimplex_4D_Fallback:                    return OpenSimplexNoise::Noise4_Fallback(seed, _x, _y, _z, _w);
