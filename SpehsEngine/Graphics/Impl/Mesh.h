...
#include "SpehsEngine/Graphics/Impl/Bone.h"
...
namespace se::gfx::impl
{
	...
	public:
		...
		Mesh() = delete;
		Mesh(Model& _model, const ModelNode& _node, const size_t _materialIndex,
			 std::shared_ptr<VertexBuffer> _vertexBuffer, std::shared_ptr<IndexBuffer> _indexBuffer);

		void addBone(const ModelNode* _boneNode, const glm::mat4& _offsetMatrix);
		virtual void updateMatrices() override;
		void clearOverriddenModelAttributes();
	...
	private:
		...
		// Needs to be a mutable reference, because graphics systems needs to register as
		// a renderer for the Model's buffers through the primitive
		Model& model;
