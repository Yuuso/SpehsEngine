vec3  v_position  : TEXCOORD7   = vec3(0.0, 0.0, 0.0);
vec3  v_normal    : NORMAL    	= vec3(0.0, 1.0, 0.0);
vec3  v_tangent   : TANGENT     = vec3(0.0, 0.0, 1.0);
vec3  v_bitangent : BITANGENT   = vec3(1.0, 0.0, 0.0);
vec4  v_color0    : COLOR0    	= vec4(1.0, 1.0, 1.0, 1.0);
vec4  v_color1    : COLOR1    	= vec4(1.0, 1.0, 1.0, 1.0);
vec4  v_color2    : COLOR2    	= vec4(1.0, 1.0, 1.0, 1.0);
vec4  v_color3    : COLOR3    	= vec4(1.0, 1.0, 1.0, 1.0);
vec2  v_texcoord0 : TEXCOORD0   = vec2(0.0, 0.0);
vec2  v_texcoord1 : TEXCOORD1   = vec2(0.0, 0.0);
vec2  v_texcoord2 : TEXCOORD2   = vec2(0.0, 0.0);
vec4  v_data0     : TEXCOORD3   = vec4(0.0, 0.0, 0.0, 0.0);
vec4  v_data1     : TEXCOORD4   = vec4(0.0, 0.0, 0.0, 0.0);
vec4  v_data2     : TEXCOORD5   = vec4(0.0, 0.0, 0.0, 0.0);
vec4  v_data3     : TEXCOORD6   = vec4(0.0, 0.0, 0.0, 0.0);
vec4  v_data4     : TEXCOORD7   = vec4(0.0, 0.0, 0.0, 0.0);

vec3  a_position  : POSITION;
vec3  a_normal    : NORMAL;
vec3  a_tangent   : TANGENT;
vec3  a_bitangent : BITANGENT;
vec4  a_color0    : COLOR0;
vec4  a_color1    : COLOR1;
vec4  a_color2    : COLOR2;
vec4  a_color3    : COLOR3;
vec4  a_weight    : WEIGHT;
ivec4 a_indices   : BLENDINDICES;
vec2  a_texcoord0 : TEXCOORD0;
vec2  a_texcoord1 : TEXCOORD1;
vec2  a_texcoord2 : TEXCOORD2;
vec4  i_data0     : TEXCOORD3;
vec4  i_data1     : TEXCOORD4;
vec4  i_data2     : TEXCOORD5;
vec4  i_data3     : TEXCOORD6;
vec4  i_data4     : TEXCOORD7;
